...
import com.sportskonnect.activities.SingleChatWindow;
...
import com.sportskonnect.helpers.NotificationsManager;
...
import com.sportskonnect.recevers.MessagesReceiverBroadcast;
...
public class MainService extends Service {
    ...
    public static void disconnectSocket() {
        ...
        try {
            ...
            json.put("senderId", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, "")));
        ...
        try {
            ...
            jsonConnected.put("connectedId", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, "")));
        } catch (JSONException | NumberFormatException e) {
            e.printStackTrace();
        }
        ...
        Log.d("CONNECTION_STATUS", "disconnect in service");
    }
    ...
    public static void connectToServer() {
        ...
        mSocket.on(Socket.EVENT_CONNECT, args -> {
            Constant.LogCat("You are connected to chat server with id " + getSocketID());
        });

        mSocket.on(Socket.EVENT_DISCONNECT, args -> {
            Constant.LogCat("You are lost connection to chat server ");

        });
        mSocket.on(Socket.EVENT_CONNECT_TIMEOUT, args -> {
            Constant.LogCat("SOCKET TIMEOUT");
            reconnect();
        });
        mSocket.on(Socket.EVENT_RECONNECT, args -> {
            Constant.LogCat("Reconnect");
            if (!mSocket.connected()) {
                mSocket.connect();

//                TODO khasni ndir custom lhad l7abs
            }
        });
        mSocket.connect();
        JSONObject json = new JSONObject();
        try {
            ...
            json.put("connectedId", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, "")));
        ...
        try {
            ...
            jsonObject.put("senderId", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, "")));
        } catch (JSONException | NumberFormatException e) {
            e.printStackTrace();
        }

        Log.d("DATA",new Gson().toJson(jsonObject)+"");
            ...
            protected void MessageReceived(Context context, Intent intent) {
                ...
                switch (action) {
                    case "new_user_message_notification":
                        ...
                        Intent messagingIntent = new Intent(WhatsCloneApplication.getAppContext(), Activity_TournamentChat.class);
                        ...
                        Constant.LogCat("recipient " + recipientId);
                        Constant.LogCat("sender " + senderId);
                        if (file != null) {
...
//                        String name = UtilsPhone.getContactName(WhatsCloneApplication.getAppContext(), userPhone);
                        ...
                        messagingIntent = new Intent(WhatsCloneApplication.getAppContext(), Activity_ChatDoubles.class);
                        messagingIntent.putExtra("conversationID", intent.getExtras().getInt("conversationID"));
    ...
    private static void isUserConnected() {
        mSocket.on(AppConstants.SOCKET_CONNECTED, args -> {
            ...
            try {
                ...
                if (connectedId != Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, ""))) {
                    if (connected) {
                        Constant.LogCat("User with id  --> " + connectedId + " is connected --> " + getSocketID() + " <---");
                        handler.postDelayed(() -> MainService.unSentMessages(connectedId), 1000);
                    } else {
                        Constant.LogCat("User with id  --> " + connectedId + " is disconnected --> " + getSocketID() + " <---");
                    }

                } else {
                    setSocketID(socketId);

                    Constant.LogCat("You  are connected with socket id --> " + getSocketID() + " <---");
                }
            } catch (JSONException | NumberFormatException e) {
...
/*                .equalTo("isFileUpload", false)*/
                .equalTo("senderID", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(), SharedprefKeys.USER_ID, "")))
                .sort("id", Sort.ASCENDING).findAll();
        Constant.LogCat("size " + messagesModelsList.size());
...
//        if (!messagesModel.isFileUpload()) return;
    ...
    private static void updateStatusDeliveredOffline() {
        ...
        List<MessagesModel> messagesModels = realm.where(MessagesModel.class)
                .notEqualTo("recipientID", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,"")))
    ...
    private static void RecipientMarkMessageAsDelivered(int messageId) {
        ...
        Constant.LogCat("--> Recipient mark message as  delivered <--");
    }
    ...
    private void SenderMarkMessageAsDelivered() {
        ...
        mSocket.on(AppConstants.SOCKET_IS_MESSAGE_DELIVERED, args -> {
            ...
            Constant.LogCat("--> Sender mark message as  delivered: update status  <--");
    ...
    private void updateDeliveredStatus(JSONObject data) {
        try {
            ...
            if (senderId == Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,""))) return;
            Realm realm = Realm.getDefaultInstance();
            realm.executeTransaction(realm1 -> {
                ...
                if (messagesModel != null) {
                    ...
                    Constant.LogCat("Delivered successfully");
                    ...
                    Constant.LogCat("Delivered failed ");
                }
            });
            realm.close();
        } catch (JSONException | NumberFormatException e) {
            Constant.LogCat("Save data to realm delivered JSONException " + e.getMessage());
        }
    }
            ...
            if (!checkIfGroupConversationExist(groupID, realm)) {
                realm.executeTransaction(realm1 -> {
                    ...
                    try {
                        ...
                        Constant.LogCat("last ID message  MainService" + lastID);

                    } catch (Exception e) {
                        Constant.LogCat("last conversation  ID  if conversation id = 0 Exception MainService" + e.getMessage());
...
/*                    here we need to apply your service api*/
...
*/
                ...
                realm.executeTransaction(realm1 -> {
                    ...
                    try {
                      ...
                      /*  if (!messagesModel.getImageFile().equals("null")) {
                        ...
                        Constant.LogCat("last conversation  ID group if conversation id = 0 Exception  MainService " + e.getMessage());
                    }
    ...
    private void onReceivedNewMessageGroup() {
        ...
        mSocket.on(AppConstants.SOCKET_NEW_MESSAGE_GROUP, args -> {
            ...
            try {
                ...
                if (recipientId == Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,""))) {
                    data.put("pingedId", recipientId);
                ...
                Constant.LogCat("New group message MainService " + e.getMessage());
            }
        });
        mSocket.on(AppConstants.SOCKET_USER_PINGED_GROUP, args -> {
            ...
            try {
                ...
                Constant.LogCat("pinged " + pinged);
                Constant.LogCat("pinged id " + pingedID);
                if (pinged) {
                    Constant.LogCat("User connected MainService (group)" + dataString.getInt("recipientId"));
                    ...
                    Constant.LogCat("User not  connected  MainService (group)" + dataString.getInt("recipientId"));
                ...
                Constant.LogCat("Group message received JSONException  MainService" + e.getMessage());
            }
        });
    ...
    private void onReceivedNewMessage() {
        ...
        mSocket.on(AppConstants.SOCKET_NEW_MESSAGE, args -> {
            ...
            Log.d("PINGE_DATA",new Gson().toJson(data));

            try {
                int recipientId = data.getInt("recipientId");
                String sender_name = data.getString("senderName");

                SharedPreference.setSharedPreferenceString(this,SharedprefKeys.NOTISENDERNAME,sender_name);


                data.put("pingedId", Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,"")));
                data.put("pinged", false);
                ...
                mSocket.emit(AppConstants.SOCKET_USER_PING, data, (Ack) argObjects -> {
                    ...
                    Log.d("PINGE_DATA_TWO",new Gson().toJson(dataString));
                    ...
                    try {
                        ...
                        if (pinged && pingedID == recipientID) {
                            Constant.LogCat("User  connected " + dataString.getInt("pingedId"));
                            mSocket.emit(AppConstants.SOCKET_IS_MESSAGE_SENT, dataString);
                            saveNewMessage(dataString);
                        } else {
                            Constant.LogCat("User not  connected " + dataString.getInt("pingedId"));

                        }
                    } catch (JSONException | NumberFormatException e) {
                        Constant.LogCat("User message received MainService JSONException" + e.getMessage());
                    }
                });
            } catch (Exception e) {
                Constant.LogCat("User received  new message  Exception MainService" + e.getMessage());
            }
        });
    ...
    private void sendPongToSender() {
        ...
        mSocket.on(AppConstants.SOCKET_IS_MESSAGE_SENT, args -> {
            ...
            try {
                ...
                if (SenderID != Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,"")))
                    return;
                updateStatusAsSentBySender(dataOn, AppConstants.IS_SENT);
            } catch (JSONException | NumberFormatException e) {
                Constant.LogCat("Recipient is online  MainService" + e.getMessage());
            }
    ...
    private int getConversationIdByGroupId(int groupId, Realm realm) {
        try {
            ...
            Constant.LogCat("Conversation id  (group) Exception MainService  " + e.getMessage());
            return 0;
        }
    }
    ...
    private void updateGroupDeliveredStatus(JSONObject data) {
        ...
        try {
            int groupId = data.getInt("groupId");
            int senderId = data.getInt("senderId");
            Constant.LogCat("groupId " + groupId);
            Constant.LogCat("sen " + senderId);
            if (senderId != Integer.parseInt(SharedPreference.getSharedPreferenceString(WhatsCloneApplication.getAppContext(),SharedprefKeys.USER_ID,""))) return;

            int ConversationID = getConversationIdByGroupId(groupId, realm);
            Constant.LogCat("conversation  id seen " + ConversationID);
                ...
                for (MessagesModel messagesModel1 : messagesModelsRealm) {
                    ...
                    realm.executeTransaction(realm1 -> {
                        ...
                        if (messagesModel != null) {
                            messagesModel.setStatus(AppConstants.IS_DELIVERED);
                            realm1.copyToRealmOrUpdate(messagesModel);
                            Constant.LogCat("Delivered successfully MainService");
                            ...
                            Constant.LogCat("Seen  failed MainService ");
                        }
                    });
            ...
            Constant.LogCat("Save to realm seen MainService " + e.getMessage());
        }
    ...
    private void updateGroupSeenStatus(JSONObject data) {
        ...
        try {
                ...
                for (MessagesModel messagesModel1 : messagesModelsRealm) {
                    ...
                    realm.executeTransaction(realm1 -> {
                        ...
                        if (messagesModel != null) {
                            ...
                            realm1.copyToRealmOrUpdate(messagesModel);
                            Constant.LogCat("seen successfully");
                            ...
                            Constant.LogCat("Seen  failed MainService (group)");
                        }
                    });
                }
            }
            ...
            Constant.LogCat("Save to realm seen " + e);
        }
    ...
    private void updateGroupSentStatus(JSONObject data) {
        ...
        try {
                ...
                for (MessagesModel messagesModel1 : messagesModelsRealm) {
                    ...
                    realm.executeTransaction(realm1 -> {
                        ...
                        if (messagesModel != null) {
                            ...
                            realm1.copyToRealmOrUpdate(messagesModel);
                            Constant.LogCat("Sent successfully MainService");
                            ...
                            Constant.LogCat("Sent  failed  MainService");
                        }
                    });
            ...
            Constant.LogCat("Save to realm sent Exception MainService " + e.getMessage());
        }
    ...
    private void updateSeenStatus(JSONObject data) {
        ...
        try {
                ...
                for (MessagesModel messagesModel1 : messagesModelsRealm) {
                    ...
                    realm.executeTransaction(realm1 -> {
                        ...
                        if (messagesModel != null) {
                            messagesModel.setStatus(AppConstants.IS_SEEN);
                            realm1.copyToRealmOrUpdate(messagesModel);
                            Constant.LogCat("Seen successfully MainService");
            ...
            Constant.LogCat("Save to realm seen  Exception" + e.getMessage());
        }
    ...
    private int getConversationId(int recipientId, int senderId, Realm realm) {
        try {
            ...
            Constant.LogCat("Conversation id Exception MainService" + e.getMessage());
            return 0;
        }
    }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Constant.LogCat(" Is sent messages Realm Error" + e.getMessage());
                }

                realm.close();

            } catch (Exception e) {
                Constant.LogCat("null object Exception MainService" + e.getMessage());
            }


        } catch (JSONException | NumberFormatException e) {
            Constant.LogCat("UpdateStatusAsSentBySender error  MainService" + e.getMessage());
        }
    }
...
//                String document = data.getString("document");
                        ...
                        try {
                         ...
                         /*   messagesModel.setFileUpload(true);
                          ...
                          /*  if (!messagesModel.getImageFile().equals("null")) {
                            ...
                            Constant.LogCat("Exception  last id messages send MainService" + e.getMessage());
                        }
            ...
            Constant.LogCat("save message Exception MainService" + e.getMessage());
        }
